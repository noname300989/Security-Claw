    ...
    questions: [
      {
        ...
        question:
          "What is AI04:2025 Data Poisoning, and how can an autonomous agent defending a network be compromised by it?",
        answer:
          "Data Poisoning involves deliberately manipulating the training data, fine-tuning data, or real-time 'Retrieval-Augmented Generation (RAG)' knowledge bases that an AI agent relies upon. If an autonomous OpenClaw Red Team or Blue Team agent dynamically ingests external threat feeds or scans local network logs into its context window, an attacker can strategically insert malicious instructions or biased artifacts into those logs (e.g., via a poisoned User-Agent string HTTP request). If the agent blindly Trusts this ingested data without sanitization, it could be tricked into ignoring real attacks or executing secondary payloads via Indirect Prompt Injection.",
      },
      {
        category: "Penetration Testing",
        question:
          "Explain the difference between a Bind Shell and a Reverse Shell. Why is a Reverse Shell almost always preferred during modern offensive engagements?",
        answer:
          "A Bind Shell occurs when the attacker executes a payload on the target machine that opens a listening port, forcing the attacker to actively connect inbound to the target. A Reverse Shell occurs when the payload on the target actively connects outbound back to the attacker's listening machine. In modern corporate environments, ingress (inbound) firewalls and NAT appliances block almost all unexpected incoming connections, making Bind Shells useless. However, egress (outbound) traffic is often poorly filtered (allowing HTTP/HTTPS/DNS out), meaning a Reverse Shell connecting out from the target to the attacker's server will successfully bypass the perimeter firewall.",
      },
      {
        category: "LLM Security",
        question:
          "What is an Insecure Output Handling vulnerability (LLM02), and how does it lead to XSS or Server-Side manipulation?",
        answer:
          "Insecure Output Handling occurs when a downstream application or internal system blindly accepts the unvalidated output of a Large Language Model and executes or renders it directly. LLMs are non-deterministic and can easily be manipulated via Prompt Injection. If an attacker injects a prompt that causes the LLM to output malicious JavaScript, and the web app renders that output to another user without sanitizing it via HTML encoding, it creates a Stored XSS vulnerability. If the downstream system uses the LLM output to form an OS command or database query, it creates Command Injection or SQLi.",
      },
...
Visit your Local Dashboard at \`apps/interview-dashboard/index.html\` to study!
  ...
  // await sendMulticastMessage(_notificationDetails, ["whatsapp", "telegram", "discord"]);
};
